...
export function BackgroundCropModal({
  ...
  const handleConfirm = async () => {
    ...
    setProcessing(true);
    try {
      const croppedBase64 = await getCroppedBackgroundImg(
        imageSrc,
        croppedAreaPixels,
        rotation,
      );
      ...
      console.error("Lỗi khi cắt ảnh background:", err);
    } finally {
      setProcessing(false);
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/75 backdrop-blur-md animate-fade-in font-sans">
      <div className="relative w-full max-w-2xl bg-white/85 dark:bg-slate-900/90 backdrop-blur-2xl border border-white/50 dark:border-slate-800/70 rounded-[32px] p-6 sm:p-7 shadow-2xl space-y-5 text-slate-900 dark:text-slate-100 max-h-[90vh] overflow-y-auto">
        {/* Header */}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-extrabold tracking-tight text-slate-900 dark:text-white">
                CẮT & CHỈNH SỬA HÌNH NỀN (BACKGROUND)
              </h2>
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Kéo di chuyển, phóng to/thu nhỏ và xoay để căn khung hình nền 16:9
              </p>
            </div>
          </div>
          <button
        ...
        {/* Cropper Workspace Canvas Container (Aspect Ratio 16:9) */}
        <div className="relative w-full h-80 rounded-2xl overflow-hidden bg-slate-950/90 border border-slate-700/60 shadow-inner">
          <Cropper
            image={imageSrc}
            crop={crop}
            zoom={zoom}
        ...
        <div className="flex items-center justify-between pt-2 border-t border-slate-200/60 dark:border-slate-800/80">
          ...
          <div className="flex items-center gap-3">
            ...
            >
              ...
              <span>{processing ? "Đang xử lý..." : "Xác nhận / Áp dụng hình nền"}</span>
            </button>
          </div>
        </div>
      </div>
...
// ---------------------------------------------------------------------------
// HTML5 Canvas Cropping Helper Function (Exports 1280x720 JPEG DataURL)
// ---------------------------------------------------------------------------

async function getCroppedBackgroundImg(
  imageSrc: string,
  pixelCrop: Area,
  rotation = 0,
): Promise<string> {
  ...
  // Export 1280x720 for optimal 16:9 Background resolution & low storage overhead
  const destCanvas = document.createElement("canvas");
  destCanvas.width = 1280;
  destCanvas.height = 720;
