...
@pytest.mark.asyncio
async def test_persist_node_ends_at_pending_review(tmp_path: Path) -> None:
    ...
    assert result == {"scenario_id": spec.scenario_id, "scenario_status": ScenarioStatus.PENDING_REVIEW}
...
async def test_persist_node_maps_write_failure_to_nonrepairable_issue(tmp_path: Path) -> None:
    repository = ScenarioRepository(make_engine(f"sqlite:///{tmp_path / 'app.db'}"))
    repository.create_schema()
    result = await persist_pending_review_node({}, repository)
...
async def test_persistence_tail_graph_converts_persists_and_ends(tmp_path: Path) -> None:
    ...
    assert result["scenario_status"] == ScenarioStatus.PENDING_REVIEW
...
def test_exported_forge_graph_has_persist_as_its_only_success_terminal() -> None:
    ...
    outgoing = {edge.source: edge.target for edge in graph.edges if edge.source == "persist_pending_review"}
    assert outgoing == {"persist_pending_review": "__end__"}
    assert "convert_xosc" in graph.nodes
    assert "persist_pending_review" in graph.nodes
