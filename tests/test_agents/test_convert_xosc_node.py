...
def make_spec(
    ...
    *,
    ...
    is_run_red_light = maneuver is ManeuverType.RUN_RED_LIGHT
    actor_s_offset = -25 if is_cut_in else (-15 if is_lane_drift else (0 if is_run_red_light else 20))
    actor_lane_offset = 0 if is_run_red_light else -1
    ...
    road_type = RoadType.URBAN_STRAIGHT if maneuver is ManeuverType.RUN_RED_LIGHT else RoadType.HIGHWAY
    return ScenarioSpec(
        scenario_id="sc_900",
        ...
        actors=[
            ...
            ActorSpec(
                name="other",
                category=category,
                position=Position(lane_offset=actor_lane_offset, s_offset_m=actor_s_offset),
...
def test_run_red_light_keeps_actor_moving_and_sets_its_signal_red() -> None:
    ...
    *Init support*, ✅ ở cột *Story support*, và code khớp với bảng:
    ...
    assert [signal.attrib for signal in signals] == [
        {"name": "id=122", "state": "RED"},
        {"name": "id=118", "state": "GREEN"},
    ]
    assert ego_position is not None and ego_position.get("x") == "217.04"
    assert actor_position is not None and actor_position.get("x") == "258.31"
    assert root.find(".//Condition[@name='criteria_RunningRedLightTest']") is None


def test_run_red_light_rejects_a_position_that_would_be_silently_ignored() -> None:
    spec = make_spec(ManeuverType.RUN_RED_LIGHT)
    actor = spec.actors[1].model_copy(update={"position": Position(lane_offset=0, s_offset_m=10)})
    spec = spec.model_copy(update={"actors": [spec.actors[0], actor]})
    with pytest.raises(ConversionError, match="perpendicular red-light approach"):
        convert_spec_to_xosc(spec)
